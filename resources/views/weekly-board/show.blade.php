<x-site-layout>
    <div class="max-w-6xl mx-auto px-4 py-8">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-neutral-900 dark:text-white mb-2">
                        {{ __('app.weekly_board') }} - {{ __('app.week_of') }} {{ $targetWeek->format('F j, Y') }}
                    </h1>
                    <p class="text-neutral-600 dark:text-neutral-300">
                        {{ $targetWeek->format('M j') }} - {{ $targetWeek->copy()->endOfWeek()->format('M j, Y') }}
                    </p>
                </div>
        ...
        <div class="grid lg:grid-cols-3 gap-8">
            <!-- Challenge Info & Daily Breakdown -->
            <div class="space-y-6">
                <!-- Challenge Details -->
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="text-right">
                                    <div class="text-sm font-medium text-neutral-900 dark:text-white">
                                        {{ $day['completions'] }} {{ __('app.completions') }}
                                    </div>
                                    @if($day['avg_time'])
                                        <div class="text-xs text-neutral-500 dark:text-neutral-400">
                                            @php($ms = (int) $day['avg_time'])
                                            @php($s = intdiv($ms, 1000))
                                            {{ sprintf('%02d:%02d avg', intdiv($s,60), $s%60) }}
                                        </div>
                                    @endif
                                </div>
                            </div>
                                    ...
                                    @foreach($leaderboard as $index => $attempt)
                                        <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-900/30">
                                            ...
                                            <td class="px-4 py-3 whitespace-nowrap text-sm text-neutral-500">
                                                {{ $attempt->completed_at?->format('M j H:i') }}
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
