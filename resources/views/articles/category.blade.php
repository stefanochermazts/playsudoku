...
@section('head')
<title>{{ $category->localized_name }} - Articoli | PlaySudoku</title>
<meta name="description" content="{{ $category->localized_description ?? 'Scopri tutti gli articoli nella categoria ' . $category->localized_name . ' su PlaySudoku.' }}">
<meta name="keywords" content="sudoku, {{ strtolower($category->localized_name) }}, articoli, guide, PlaySudoku">
...
<!-- Canonical URL -->
<link rel="canonical" href="{{ route('articles.category', ['locale' => app()->getLocale(), 'category' => $category->slug]) }}">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div class="lg:col-span-3">
                ...
                <div class="bg-white/80 dark:bg-neutral-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-neutral-200/50 dark:border-neutral-700/50 p-6 mb-8">
                    <form method="GET" action="{{ route('articles.category', ['locale' => app()->getLocale(), 'category' => $category->slug]) }}" class="flex flex-col sm:flex-row gap-4">
                        {{-- Search --}}
                        <div class="flex-1">
                            <input type="text" 
                                   name="search" 
                    ...
                    <div class="space-y-2">
                        ...
                        @foreach($categories as $cat)
                            @if($cat->id !== $category->id)
                                <a href="{{ route('articles.category', ['locale' => app()->getLocale(), 'category' => $cat->slug]) }}" 
